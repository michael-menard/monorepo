...
commands:
  ...
  - develop-story:
      - branch-creation: 'BEFORE any implementation, create a semantic feature branch using the format: {type}/{story-number}-{short-kebab-description}. Branch type MUST be one of: feature/ (new functionality), bugfix/ (bug fixes), task/ (technical tasks, refactoring, chores), hotfix/ (urgent production fixes). Determine type from story content and acceptance criteria. Examples: feature/3.1.47-delete-schema-updates, bugfix/3.1.48-fix-upload-validation, task/3.1.49-refactor-auth-service. Command: git checkout -b {type}/{story-number}-{short-kebab-description}. If branch already exists from prior session, checkout that branch instead. NEVER develop directly on main. CRITICAL: Always use semantic branch prefixes - never use generic "story/" prefix.'
      ...
      - commit-and-push: 'Run linter with auto-fix: pnpm lint --fix→Stage all changes with git add -A→Commit with message format: feat(story-number): story title (e.g., "feat(3.1.47): delete database schema updates")→Push branch to origin with git push -u origin {branch-name}. CRITICAL: NEVER use --no-verify flag on git commit or git push. This flag bypasses pre-commit/pre-push hooks and is STRICTLY FORBIDDEN unless explicitly approved by user. If hooks fail, fix the underlying issues instead of bypassing them.'
      - create-pr: 'Create PR using: gh pr create --title "Story {story-number}: {story-title}" --body "## Story\n\n{link-to-story-file}\n\n## Changes\n\n{summary-of-changes-from-File-List}\n\n## Testing\n\n- [ ] All tests pass\n- [ ] Manual testing completed" --base main. After PR creation, enable auto-merge with: gh pr merge {pr-number} --auto --squash. This allows the PR to automatically merge after QA approval and all checks pass. If gh CLI not available, provide the GitHub PR URL for manual creation.'
