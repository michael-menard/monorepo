# Story 3.1.24: Expiry & Interrupted Uploads — Auto-Refresh & Resume

## Status

Ready for Review

## Story

As a creator,
I want uploads to recover from expiry and interruptions,
so I can finish without starting over.

## Acceptance Criteria

1. Expiry detection

- Detect expired upload session via API error code EXPIRED_SESSION or local ttl > sessionTtlMinutes; mark files expired.

2. Auto-refresh flow

- "Refresh session" re-creates the upload session or re-registers affected files; preserve successful uploads; do not re-upload them.

3. Resume interrupted

- Allow retry for failed/canceled with existing File handle when available; otherwise prompt re-select.

4. Tests

- upload manager: expiry detection and refresh; resume flows.

## Tasks / Subtasks

- [x] Implement refresh in useUploadManager
- [x] UI hooks to trigger refresh and retries
- [x] Tests

## Dev Notes

- Respect upload session TTL from 3.1.8; store issuedAt per session to estimate staleness.

### Auth, Cookies, and CSRF

- Cookie-based (HttpOnly) session auth.
- Frontend MUST send `credentials: 'include'` and MUST NOT send `Authorization` headers.
- Cookie attributes: `HttpOnly`; `Secure`; `SameSite=Lax` (recommended) or `SameSite=None; Secure` when API is on a different site/domain.
- CORS: `Access-Control-Allow-Credentials: true`; `Access-Control-Allow-Origin` must be the exact client origin (no `*`). Include `Vary: Origin`.
- Responses are JSON; clients send `Accept: application/json`.
- CSRF for unsafe methods (POST/PUT/PATCH/DELETE): require `X-CSRF-Token` and verify server-side (double-submit cookie or session-bound token). GET endpoints do not require CSRF.

Ensure:

- RTK Query uses `credentials: 'include'` and does not set `Authorization`.
- XHR uploads use `withCredentials = true` and appropriate `Content-Type` (e.g., `application/octet-stream` for binary part uploads).
- Backend sets cookies with `HttpOnly`; `Secure`; `SameSite` (Lax or None+Secure) and CORS allows credentials for the exact origin.
- Optional `X-CSRF-Token` header is included on unsafe methods when CSRF is enabled.

## Testing

- [x] Session management tests (setSession, isSessionExpired)
- [x] Expiry detection before upload (startUploads, retry blocked when expired)
- [x] Auto-refresh flow (updateFileUrls, markExpiredFiles, preserves successful uploads)
- [x] File handle detection (hasFileHandle, getFilesNeedingReselect, onFileNeedsReselect)
- [x] 34 tests passing (21 existing + 13 new for Story 3.1.24)

## Dev Agent Record

### Agent Model Used

Claude Opus 4.5

### Debug Log References

N/A

### Completion Notes

- Added local TTL expiry detection with 30-second buffer before actual expiry
- Implemented `setSession(sessionId, expiresAt)` to store session info
- Implemented `isSessionExpired()` for local TTL check
- Added `updateFileUrls()` for refresh flow - updates URLs and resets expired files to queued
- Added `markExpiredFiles()` to mark all non-complete files as expired
- Added `hasFileHandle()` and `getFilesNeedingReselect()` for resume detection
- Added `onFileNeedsReselect` callback for UI to prompt re-selection
- Updated `startUploads()` to check expiry before starting, returns false if expired
- Updated `retry()` to check expiry and file handle, returns false if blocked
- Updated `retryAll()` to return list of files needing re-selection
- Preserves successful uploads when marking expired (AC2 requirement)
- 13 new tests covering all new functionality

### File List

| File | Action |
| ---- | ------ |
| apps/web/main-app/src/hooks/useUploadManager.ts | Modified |
| apps/web/main-app/src/hooks/__tests__/useUploadManager.test.tsx | Modified |

## Change Log

| Date       | Version | Description                           | Author    |
| ---------- | ------- | ------------------------------------- | --------- |
| 2025-12-06 | 0.1     | Initial draft (expiry/resume uploads) | SM        |
| 2025-12-26 | 1.0     | Implementation complete               | Dev Agent |
