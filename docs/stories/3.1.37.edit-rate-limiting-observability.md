...
- [ ] **Task 1: Configure Shared Rate Limiting** (AC: 1)
  - [ ] Verify edit presign uses same rate limit key as upload: `moc-upload:{userId}:{date}`
  - [ ] Only count finalize operations against quota (not presign)
  - [ ] Document shared quota in API documentation

- [ ] **Task 2: Add Structured Log Events** (AC: 2, 3)
  - [ ] Add `moc.edit.start` log to existing GET handler (`get/handler.ts`) when `isOwner: true`
  - [ ] Add `moc.edit.presign` log when presign URLs are generated
  - [ ] Add `moc.edit.finalize` log when changes are committed
  - [ ] Include: correlationId, requestId, ownerId, mocId
  - [ ] Include: file counts by category, metadata fields changed

- [ ] **Task 3: Enhance 429 Response** (AC: 4)
  - [ ] Include `retryAfterSeconds` in response body
  - [ ] Include `resetAt` timestamp (UTC)
  - [ ] Include `Retry-After` HTTP header (seconds)
  - [ ] Include current usage count and limit

- [ ] **Task 4: Audit Logs for PII** (AC: 5)
  - [ ] Review all log statements in edit handlers
  - [ ] Ensure no email, name, or other PII in logs
  - [ ] ownerId is acceptable (not considered PII for our purposes)
  - [ ] Ensure file names don't leak into logs (could contain PII)
...
- `apps/api/endpoints/moc-instructions/edit-presign/handler.ts` - Modified (add logging)
- `apps/api/endpoints/moc-instructions/edit-finalize/handler.ts` - Modified (add logging)
- `apps/api/core/utils/responses.ts` - Modified (enhance 429 response)
