...
- [x] **Task 1: Verify Edit Path Prefix** (AC: 1)
  - [x] Confirm presign uses `edit/` path prefix
  - [x] Document S3 key format in code comments

- [x] **Task 2: Implement Finalize Success Handling** (AC: 2)
  - [x] Option A: Move files from `edit/` to permanent path
  - [x] Update file records with final S3 key

- [x] **Task 3: Synchronous Cleanup on Failure** (AC: 6)
  - [x] On finalize transaction rollback, attempt to delete uploaded files
  - [x] Use try-catch to make cleanup best-effort
  - [x] Log failures as warnings (don't fail the error response)

- [x] **Task 4: Create Cleanup Lambda** (AC: 4, 5, 7)
  - [x] Create `apps/api/endpoints/cleanup/edit-orphans/handler.ts`
  - [x] List objects in `*/moc-instructions/*/edit/*` prefix
  - [x] Filter to objects older than 24 hours
  - [x] Use S3 DeleteObjects for batch deletion
  - [x] Log each deleted object with reason

- [x] **Task 5: Schedule Cleanup Job** (AC: 4)
  - [x] Add EventBridge rule for daily execution
  - [x] Configure to run at 03:00 UTC (low traffic)
  - [x] Add to serverless.yml

- [x] **Task 6: Add Observability**
  - [x] Log cleanup start with environment
  - [x] Log objects found for cleanup
  - [x] Log successful deletions
  - [x] Log any errors during cleanup
  - [ ] Emit CloudWatch metric for orphan count (deferred as optional)
...
1. **Edit Path Prefix (AC: 1)**: Verified presign uses `edit/` path prefix in S3 key format
2. **Finalize Success Handling (AC: 2)**: Files are moved from `edit/` to permanent path after successful transaction using S3 CopyObject + DeleteObject
3. **Finalize Failure Handling (AC: 3)**: Files remain in `edit/` prefix when transaction fails (orphans)
4. **Scheduled Cleanup (AC: 4)**: Daily Lambda at 03:00 UTC deletes files in `edit/` prefix older than 24 hours
5. **Cleanup Logging (AC: 5)**: Each deleted file logged with correlationId, s3Key, and ageHours
6. **Synchronous Cleanup (AC: 6)**: Best-effort cleanup on transaction failure using try-catch
7. **Batch Delete (AC: 7)**: Uses S3 DeleteObjects with 1000 object batch limit

CloudWatch metrics deferred as optional enhancement.

Tests: 26 passing (10 cleanup + 16 edit-finalize)

### File List

- `apps/api/endpoints/cleanup/edit-orphans/handler.ts` - New (cleanup Lambda)
- `apps/api/endpoints/cleanup/edit-orphans/__tests__/handler.test.ts` - New (10 tests)
- `apps/api/endpoints/moc-instructions/edit-finalize/handler.ts` - Modified (add file move, sync cleanup)
- `apps/api/serverless.yml` - Modified (add scheduled cleanupEditOrphans function)
