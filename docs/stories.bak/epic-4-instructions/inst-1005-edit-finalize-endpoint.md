...
- [x] **Task 1: Create Handler Structure** (AC: 1)
  - [x] Create `apps/api/endpoints/moc-instructions/edit-finalize/handler.ts`
  - [x] Create Zod request schema
  - [x] Create Zod response schema

- [x] **Task 2: Verify New Files in S3** (AC: 2)
  - [x] For each new file key, call S3 HeadObject
  - [x] Download first 8KB for magic bytes validation
  - [x] Verify MIME type matches expected for category
  - [x] Return 400 if any file missing or invalid

- [x] **Task 3: Soft-Delete Removed Files** (AC: 3)
  - [x] For each removed file ID, set `deletedAt = NOW()`
  - [x] Verify file belongs to this MOC (security)
  - [x] Do not hard delete - cleanup job handles that

- [x] **Task 4: Atomic Update** (AC: 4, 5)
  - [x] Use database transaction
  - [x] Update MOC metadata
  - [x] Insert new file records
  - [x] Soft-delete removed files
  - [x] Use `updatedAt` comparison for idempotency
  - [x] If `updatedAt` mismatch, return 409 (concurrent edit)

- [x] **Task 5: Re-index OpenSearch** (AC: 6)
  - [x] After successful commit, update OpenSearch
  - [x] Update all indexed fields including file references
  - [x] Fail-open with warning log

- [x] **Task 6: Return Updated Data** (AC: 7)
  - [x] Return full MOC data with updated fields
  - [x] Include new file list (excluding deleted)
  - [x] Include presigned GET URLs for files

- [x] **Task 7: Add Observability**
  - [x] Log finalize start with file counts
  - [x] Log S3 verification results
  - [x] Log transaction completion
  - [x] Warn on OpenSearch failures
...
- POST `/mocs/:mocId/edit/finalize` endpoint created
- S3 file verification with magic bytes validation
- Soft-delete for removed files (sets deletedAt)
- Atomic database transaction with optimistic locking
- Idempotent via updatedAt comparison (returns 409 on mismatch)
- OpenSearch re-indexing with fail-open pattern
- Returns updated MOC data with presigned file URLs

16 tests passing covering authentication, authorization, validation, concurrent edit detection, file operations, and response structure.

### File List

- `apps/api/endpoints/moc-instructions/edit-finalize/handler.ts` - New
- `apps/api/endpoints/moc-instructions/__tests__/edit-finalize.handler.test.ts` - New
- `apps/api/serverless.yml` - Modified (added mocEditFinalize function)
