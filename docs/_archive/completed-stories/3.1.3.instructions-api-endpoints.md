...
1. ✅ GET /api/instructions — owner-only list with pagination
   - Returns ONLY instructions belonging to the authenticated user (owner).
   - Filters (q, tags, theme) and sorting apply within the owner scope.
   - 401 if unauthenticated.
2. ✅ GET /api/instructions/:id — owner-only detail
   - 200 only if the requesting user is the owner.
   - 403 for non-owners (Forbidden). 401 if unauthenticated.
3. ✅ Error contract and observability
   - Errors follow 3.1.21; logs include requestId and ownerId.
4. ✅ RTK Query hooks generated using cookie-based auth
   - Frontend sends cookies with requests (credentials: 'include'); no Authorization header.
...
export const instructionsApi = createApi({
  ...
  baseQuery: fetchBaseQuery({
    ...
    prepareHeaders: headers => {
      ...
      // Do NOT set Authorization; cookie-based auth only
      return headers
    },
  }),
...
export const handler = async (event: APIGatewayProxyEvent) => {
  // Require cookie-based auth; extract ownerId from session
  const session = await getSessionFromCookies(event)
  ...
  const baseWhere = and(
    eq(instructions.ownerId, ownerId), // OWNER-ONLY SCOPE
    q ? or(like(instructions.name, `%${q}%`), like(instructions.description, `%${q}%`)) : undefined,
...
- [ ] API test: owner-only isolation (list)  ensure another user's records are excluded
- [ ] API test: owner-only detail  GET /instructions/:id returns 403 for non-owner
