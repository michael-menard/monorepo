    ...
    <Card className="bg-card border-border dark:backdrop-blur-sm dark:hover:border-primary/30 transition-all duration-200">
      ...
      <CardContent className="px-4 md:px-6">
        <div className="relative space-y-3 md:space-y-4">
          <div
            className="absolute left-3.5 md:left-4 top-2 bottom-2 w-px bg-border"
            aria-hidden="true"
          />
          ...
          {activities.slice(0, 5).map(activity => {
            ...
            return (
              <div key={activity.id} className="relative flex items-start gap-3 md:gap-4 pl-0">
                <div
                  className={`relative z-10 flex h-7 w-7 md:h-8 md:w-8 flex-shrink-0 items-center justify-center rounded-full ${activityColors[activity.type]}`}
                >
