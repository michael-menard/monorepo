...
updated_at: "2026-02-08T22:15:00-07:00"
...
**Criterion**: Implementation must follow the canonical hexagonal architecture pattern documented in `docs/architecture/api-layer.md` using `domains/*/application/` structure.

**Verification**:
- All code placed in `apps/api/lego-api/domains/wishlist/application/`, `adapters/`, `ports/` per existing pattern
- Consistent with all 14 existing domains (gallery, wishlist, health, instructions, sets, parts-lists, config, admin, auth, authorization, inspiration, mocs)
- No new code in legacy `apps/api/services/` pattern

**Notes**: QA audit identified perceived mismatch with docs, but verification confirms docs already document `domains/` as canonical pattern (line 10 of api-layer.md). All existing domains use this pattern. No doc update needed - implementation should proceed with documented pattern.
...
**Query String Format**:
- Store filter: `?store=LEGO,BrickLink` (comma-separated, parsed to `string[]`)
- Priority range: `?priority=3,5` (comma-separated min,max, parsed to `{ min: 3, max: 5 }`)
- Price range: `?priceRange=50,200` (comma-separated min,max, parsed to `{ min: 50, max: 200 }`)
- Combined example: `?store=LEGO&priority=3,5&priceRange=50,200&sort=bestValue`

**Verification**:
- Unit tests validate schema accepts valid parameters
- Unit tests validate schema rejects invalid parameters (including invalid format, min > max)
...
- ✅ `GET /api/wishlist` endpoint (extend with filter parameters)
- ✅ Smart sort algorithms: bestValue, expiringSoon, hiddenGems (from WISH-2014)
- ✅ Zod schema validation (extend existing query schema)
- ✅ Drizzle query builder for database filtering + sorting

### Existing Patterns

- ✅ Error handling patterns from WISH-2001
- ✅ Repository pattern from existing wishlist domain
- ✅ Hexagonal architecture (ports & adapters)

### New Components (to build)

- 🆕 Extended `ListWishlistQuerySchema` with filter parameters
- 🆕 Repository methods for combined WHERE + ORDER BY queries
...
2. **Schema Synchronization (Frontend ↔ Backend)**
...
All 9 acceptance criteria (AC0, AC1-AC6, AC15-AC16, AC18) address core user journey requirements.

### Non-Blocking Items (Logged to KB)

| # | Finding | Category | KB Entry |
|---|---------|----------|----------|
| 1 | Cursor-based pagination for large datasets | Performance | deferred-kb-1 |
| 2 | Filter combination validation (min <= max) | Edge-Cases | deferred-kb-2 |
| 3 | Partial filter matches (LIKE queries) | Edge-Cases | deferred-kb-3 |
| 4 | Multi-value null handling strategy | Edge-Cases | deferred-kb-4 |
| 5 | Filter preset persistence | UX-Polish | deferred-kb-5 |
| 6 | Query result caching (Redis) | Performance | deferred-kb-6 |
| 7 | GraphQL-style field selection | API-Evolution | deferred-kb-7 |
| 8 | Faceted search counts (filter previews) | UX-Polish | deferred-kb-8 |
| 9 | Smart sort algorithm explanations | UX-Polish | deferred-kb-9 |
| 10 | Filter analytics/telemetry | Observability | deferred-kb-10 |
| 11 | Database composite index optimization | Performance | deferred-kb-11 |
| 12 | Filter state persistence (URL query params) | UX-Polish | deferred-kb-12 |
| 13 | Combined filter validation rules | UX-Polish | deferred-kb-13 |
| 14 | Filter auto-suggest based on existing data | UX-Polish | deferred-kb-14 |

### Summary

- **ACs added**: 0
- **KB entries created**: 14 non-blocking findings deferred to post-MVP phases (5-8)
- **Mode**: Autonomous (no PM review required)
- **Status**: Story implementation-ready with no changes to acceptance criteria

---

## Revision History

### 2026-02-08: Post-QA Elaboration Fixes (Status: deferred → backlog)

**Issues Fixed**:
1. **AC0 - Architecture Pattern Clarification**: Updated AC0 to reflect verification that `docs/architecture/api-layer.md` already documents `domains/` as canonical pattern (line 10). All 14 existing domains use this pattern. No documentation conflict exists - implementation should proceed with documented hexagonal architecture pattern.

2. **AC1 - Query Parameter Format Specification**: Added explicit query string format specification:
   - Store: `?store=LEGO,BrickLink` → `string[]`
   - Priority: `?priority=3,5` → `{ min: 3, max: 5 }`
   - Price: `?priceRange=50,200` → `{ min: 50, max: 200 }`
   - Combined: `?store=LEGO&priority=3,5&priceRange=50,200&sort=bestValue`

**Status Change**: deferred → backlog (ready for re-audit and implementation)

**QA Audit Result**: CONDITIONAL PASS with 2 MUST FIX issues (both resolved)
