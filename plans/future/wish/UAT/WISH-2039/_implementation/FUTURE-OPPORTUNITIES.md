...
| 4 | ~~Cache key pattern not explicitly documented~~ | — | — | RESOLVED in re-elaboration: User overrides will be cached with flag data (single cache key per environment). AC6 clarified to cache user overrides in flag cache entry. |
