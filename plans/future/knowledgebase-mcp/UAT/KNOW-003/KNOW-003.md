...
### Gaps Identified (User Accepted as AC Enhancements)
...
| 2 | kb_update doesn't fetch existing entry to validate it exists before re-embedding | Add as AC | Must fetch BEFORE generating new embedding to avoid wasted API calls on non-existent entries. |
| 3 | No monitoring/observability for embedding cache hit rates | Add as AC | Add cache hit/miss metrics to logger.info calls to validate caching effectiveness. |
| 4 | Concurrent kb_update on same entry could generate embeddings twice unnecessarily | Add as AC | Document acceptable race condition in Concurrent Operations section. |
| 5 | No explicit handling for null vs undefined vs omitted parameters in kb_update | Add as AC | Clarify Zod schema should validate at least one field is defined and not undefined. |
| 6 | Test plan doesn't include testing with actual OpenAI API vs mocked client | Add as AC | Include at least 1-2 integration tests with real OpenAI API. Document when to mock vs use real API. |
| 7 | No error handling documented for database connection failures during CRUD operations | Add as AC | Specify behavior when db is unavailable. Should circuit breakers or connection pool exhaustion be handled specially? |
| 8 | Performance targets don't specify whether they include cache warmup | Add as AC | Specify performance targets for both cache miss (first call) and cache hit (subsequent) scenarios. |
| 9 | Tag filtering with empty array [] vs null vs undefined has subtle differences | Add as AC | Clarify kb_list behavior when filtering with tags=[]. |
| 10 | No explicit sanitization of SQL injection vectors in tags array | Add as AC | Document that tags array elements are sanitized. Add test case with malicious SQL in tags. |

### Enhancement Opportunities (User Accepted as AC Enhancements)

| # | Finding | User Decision | Notes |
|---|---------|---------------|-------|
| 1 | Add kb_get_many for batch retrieval by IDs | Add as AC | Common use case: retrieve multiple entries by ID list efficiently. Could reduce latency by 10-50x. |
| 2 | Return created/updated entry object from kb_add instead of just UUID | Add as AC | Returning full KnowledgeEntry object is more ergonomic. Simple change to AC1. |
| 3 | Add dry-run mode for kb_update to preview what would change | Add as AC | Pass dryRun: true flag to see what would be updated without committing. |
| 4 | Implement soft delete instead of hard delete for kb_delete | Add as AC | Soft delete (deletedAt timestamp) enables undo, audit trails, and recovery. May be critical for production. |
| 5 | Add content similarity check before creating duplicate entries | Add as AC | Add optional deduplication flag to prevent truly duplicate entries. Helps with data quality. |
| 6 | Support upsert operation (create or update based on content hash) | Add as AC | Common pattern: "add if doesn't exist, update if exists". Could add kb_upsert for convenience. |
| 7 | Add last_accessed_at timestamp for kb_get to track usage patterns | Add as AC | Track which entries are frequently accessed. Could inform future ranking algorithms. |
| 8 | Implement cursor-based pagination for kb_list instead of just limit | Add as AC | Cursor-based pagination enables proper pagination through large result sets. Important for scaling. |
| 9 | Add bulk operations: kb_add_many, kb_update_many, kb_delete_many | Add as AC | Valuable for admin tasks and seeding. Could leverage EmbeddingClient batch processing. |
| 10 | Expose embedding vector in kb_list results (optional flag) | Add as AC | Add includeEmbeddings: boolean flag to reduce memory/bandwidth for large result sets. |
| 11 | Add validation for duplicate tags in tags array | Add as AC | Could add Zod refinement to enforce unique tags. Minor quality improvement. |
| 12 | Create typed error classes (NotFoundError, ValidationError) instead of generic Error | Add as AC | Creating custom error classes improves error handling and enables type-safe catch blocks. |
