...
- [ ] **AC-11**: Docker Compose `stop_grace_period` for supervisor service set to `drainTimeoutMs + 30s` buffer (default: `330s` for 5min drain + 30s buffer) in `infra/langgraph-server/compose.langgraph-server.yaml`. Without this, Docker sends SIGKILL before drain completes, defeating the purpose of graceful shutdown.
