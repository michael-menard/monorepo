...
updated_at: "2026-01-18T21:27:00-07:00"
depends_on:
  - STORY-006
tags:
  - vercel
...
The Vercel migration is progressing through the API surface. STORY-006 establishes album CRUD operations. This story migrates the image reading capabilities that allow users to browse, search, and flag inspiration gallery images.
...
- Search images using OpenSearch (with PostgreSQL fallback)
...
|----------|-------------|
| `packages/backend/gallery-images-core/` | NEW — core logic package |
| `apps/api/platforms/vercel/api/gallery/images/` | NEW — Vercel handlers |
| `apps/api/platforms/vercel/vercel.json` | MODIFY — add routes |
| `apps/api/core/database/seeds/gallery.ts` | NEW — seed data |
| `__http__/gallery.http` | NEW — HTTP test file |
...
### AC-5: Core Package Structure
- [ ] `packages/backend/gallery-images-core/` package created
- [ ] Exports: `getGalleryImage`, `listGalleryImages`, `searchGalleryImages`, `flagGalleryImage`
- [ ] Zod schemas exported from `__types__/index.ts`
- [ ] Unit tests for core logic
- [ ] Package follows `sets-core` pattern
...
- [ ] `__http__/gallery.http` created with all required requests
...
|---------|--------------|
| `packages/backend/gallery-images-core` | Core logic extraction following ports & adapters pattern. Handlers in `apps/api/platforms/vercel/` are thin adapters calling this core. |
...
┌─────────────────────────────────────────────────────────────┐
│                    Core Logic (Port)                        │
│  packages/backend/gallery-images-core/src/get-image.ts      │
│                                                             │
│  - Validate input (Zod)                                     │
...
| HP-4 | Search images returns matching results | `.http` response |
