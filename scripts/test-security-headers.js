...
async function testSecurityHeaders() {
  ...
  try {
    ...
    const setCookieHeaders = Object.keys(headers).filter(key => key.toLowerCase() === 'set-cookie')

    if (setCookieHeaders.length > 0) {
      const hasSecureCookies = setCookieHeaders.some(
        key => headers[key].includes('Secure') && headers[key].includes('HttpOnly'),
      )
