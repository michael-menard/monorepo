...
async function validateSwagger() {
  ...
  try {
    ...
    const pathsContent = fs.readFileSync(
    ...
    const checks = [
      ...
      },
      {
        ...
        test: () =>
          allContent.includes('multipart/form-data') && allContent.includes('instructionsFile:'),
    ...
    if (failed === 0) {
      ...
      console.log('\n📚 Next steps:')
      console.log(
        '1. View the documentation at: http://localhost:9000/docs (when server is running)',
      )
