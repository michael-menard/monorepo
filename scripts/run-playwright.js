...
async function checkServices() {
  ...
  const frontendUrls = ['http://localhost:3000', 'http://localhost:3001', 'http://localhost:5173']
...
function validateOptions() {
  ...
  if (options.browser && !validBrowsers.includes(options.browser)) {
    console.error(`Invalid browser: ${options.browser}. Valid options: ${validBrowsers.join(', ')}`)
    process.exit(1)
  }
