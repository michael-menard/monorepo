...
jest.mock('../controllers/auth.controller', () => ({
  ...
  signup: jest.fn((req, res) => res.status(201).json({ message: 'Signup successful' })),
  ...
  checkAuth: jest.fn((req, res) => res.status(200).json({ message: 'Authenticated' })),
}));
...
describe('Auth Routes', () => {
  ...
  describe('Auth Middleware', () => {
    it('should call verifyToken middleware', async () => {
      ...
      await verifyToken(mockRequest as Request, mockResponse as Response, jest.fn());
