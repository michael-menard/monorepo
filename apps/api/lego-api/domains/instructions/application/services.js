...
import { generateInstructionFileKey, generateThumbnailKey, generatePartsListKey, } from '../adapters/storage.js';
/**
 ...
 */
export function createInstructionsService(deps) {
    ...
    return {
         ...
         */
        async getMocWithFiles(userId, mocId) {
         ...
         */
        async listMocs(userId, pagination, filters) {
            return instructionRepo.findByUserId(userId, pagination, filters);
        },
        /**
         * Update a MOC
         */
        async updateMoc(userId, mocId, input) {
         ...
         */
        async uploadInstructionFile(userId, mocId, file) {
            // Check MOC ownership
            const mocResult = await this.getMoc(userId, mocId);
            if (!mocResult.ok) {
                return mocResult;
         ...
         * Delete a file
         */
        async deleteFile(userId, mocId, fileId) {
            // Check MOC ownership
            const mocResult = await this.getMoc(userId, mocId);
            if (!mocResult.ok) {
                return mocResult;
         ...
         */
        async listFiles(userId, mocId, fileType) {
            // Check MOC ownership
            const mocResult = await this.getMoc(userId, mocId);
            if (!mocResult.ok) {
                return mocResult;
