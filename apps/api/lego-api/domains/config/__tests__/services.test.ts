...
import type { FeatureFlagRepository, FeatureFlagCache, CachedFeatureFlags } from '../ports/index.js'
...
function createMockFlagRepo(): FeatureFlagRepository {
  return {
    ...
    update: vi.fn().mockImplementation((flagKey: string, input) => {
      const flag = mockFlags.find(f => f.flagKey === flagKey)
      if (!flag) return { ok: false, error: 'NOT_FOUND' }
      return {
        ok: true,
