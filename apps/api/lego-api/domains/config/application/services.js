...
export function createFeatureFlagService(deps) {
     ...
     */
    async function loadFlags(environment) {
        // Check cache first
    ...
    return {
         ...
         */
        async evaluateFlag(flagKey, userId, environment = 'production') {
         ...
         */
        async evaluateAllFlags(userId, environment = 'production') {
            const flags = await loadFlags(environment);
            const result = {};
         ...
         */
        async getAllFlags(userId, environment = 'production') {
            return this.evaluateAllFlags(userId, environment);
        },
        /**
         * Get a single flag with metadata
         */
        async getFlag(flagKey, environment = 'production') {
         ...
         */
        async updateFlag(flagKey, input, environment = 'production') {
         ...
         */
        invalidateCache(environment = 'production') {
            cache.invalidate(environment);
        },
        /**
