...
import { CreateImageInputSchema, UpdateImageInputSchema, ListImagesQuerySchema, CreateAlbumInputSchema, UpdateAlbumInputSchema, ListAlbumsQuerySchema, } from './types.js';
// ─────────────────────────────────────────────────────────────────────────
...
gallery.get('/images', async (c) => {
    ...
    const result = await galleryService.listImages(userId, { page, limit }, { search, albumId: albumId ?? null });
...
gallery.post('/images', async (c) => {
    ...
    if (!result.ok) {
        const status = result.error === 'INVALID_FILE' ? 400 :
...
gallery.post('/albums', async (c) => {
    ...
    if (!result.ok) {
        const status = result.error === 'VALIDATION_ERROR' ? 400 :
