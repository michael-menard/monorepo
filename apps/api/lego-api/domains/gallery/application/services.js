 ...
 */
export function createGalleryService(deps) {
    ...
    return {
         ...
         * Upload a new image
         */
        async uploadImage(userId, file, input) {
            // Validate file type
         ...
         */
        async listImages(userId, pagination, filters) {
            return imageRepo.findByUserId(userId, pagination, filters);
        },
        /**
         * Update an image
         */
        async updateImage(userId, imageId, input) {
         ...
         * Create a new album
         */
        async createAlbum(userId, input) {
         ...
         */
        async getAlbumWithImages(userId, albumId) {
            const albumResult = await this.getAlbum(userId, albumId);
            if (!albumResult.ok) {
                return albumResult;
            }
            const imagesResult = await imageRepo.findByUserId(userId, { page: 1, limit: 100 }, { albumId });
            return ok({
         ...
         */
        async listAlbums(userId, pagination, filters) {
            return albumRepo.findByUserId(userId, pagination, filters);
        },
        /**
         * Update an album
         */
        async updateAlbum(userId, albumId, input) {
            // Check ownership first
