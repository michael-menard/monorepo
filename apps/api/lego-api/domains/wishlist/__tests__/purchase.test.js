...
function createMockSetsService() {
    return {
...
describe('WishlistService.markAsPurchased', () => {
    ...
    describe('Happy Path', () => {
        it('creates Set with correct data from wishlist item', async () => {
            ...
            expect(setsService.createSet).toHaveBeenCalledWith('user-123', expect.objectContaining({
        ...
        it('copies image to Sets S3 key when imageUrl exists', async () => {
            ...
            expect(imageStorage.copyImage).toHaveBeenCalledWith('wishlist/user-123/75192.webp', expect.stringContaining('sets/user-123/'));
        });
        ...
        it('uses default purchase date (today) when not provided', async () => {
            ...
            const purchaseDate = callArgs[1].purchaseDate;
