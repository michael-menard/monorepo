 ...
 */
function createMockImageStorage(overrides = {}) {
    return {
...
describe('WishlistService', () => {
    ...
    describe('listItems', () => {
        it('returns paginated items for user', async () => {
            ...
            expect(wishlistRepo.findByUserId).toHaveBeenCalledWith('user-123', { page: 1, limit: 20 }, undefined);
        });
        it('passes filters to repository', async () => {
            await service.listItems('user-123', { page: 1, limit: 20 }, { store: 'LEGO', priority: 5 });
            expect(wishlistRepo.findByUserId).toHaveBeenCalledWith('user-123', { page: 1, limit: 20 }, { store: 'LEGO', priority: 5 });
        });
    });
    describe('createItem', () => {
        it('creates item with auto-incremented sortOrder', async () => {
            ...
            expect(result.ok).toBe(true);
            expect(wishlistRepo.insert).toHaveBeenCalledWith(expect.objectContaining({
                userId: 'user-123',
                title: 'New Set',
                store: 'LEGO',
    ...
    describe('generateImageUploadUrl', () => {
        it('generates presigned URL with imageStorage configured', async () => {
            ...
            const result = await serviceWithStorage.generateImageUploadUrl('user-123', 'test.jpg', 'image/jpeg');
            expect(result.ok).toBe(true);
            if (result.ok) {
            ...
            expect(mockImageStorage.generateUploadUrl).toHaveBeenCalledWith('user-123', 'test.jpg', 'image/jpeg', undefined);
        });
        it('returns PRESIGN_FAILED when imageStorage is not configured', async () => {
            ...
            const result = await serviceWithoutStorage.generateImageUploadUrl('user-123', 'test.jpg', 'image/jpeg');
            expect(result.ok).toBe(false);
            if (!result.ok) {
        ...
        it('propagates INVALID_EXTENSION error from storage adapter', async () => {
            ...
            const result = await serviceWithStorage.generateImageUploadUrl('user-123', 'test.pdf', 'application/pdf');
            expect(result.ok).toBe(false);
            if (!result.ok) {
        ...
        it('propagates INVALID_MIME_TYPE error from storage adapter', async () => {
            ...
            const result = await serviceWithStorage.generateImageUploadUrl('user-123', 'test.jpg', 'video/mp4');
            expect(result.ok).toBe(false);
            if (!result.ok) {
