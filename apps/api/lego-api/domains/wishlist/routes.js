...
import { createSetRepository, createSetImageRepository, createImageStorage, } from '../sets/adapters/index.js';
...
import { CreateWishlistItemInputSchema, UpdateWishlistItemInputSchema, ListWishlistQuerySchema, ReorderWishlistInputSchema, PresignRequestSchema, MarkAsPurchasedInputSchema, } from './types.js';
 ...
 */
function logAuthorizationFailure(userId, itemId, endpoint, method, statusCode, errorCode) {
...
wishlist.get('/', async (c) => {
    ...
    const tagList = tags
        ? tags
            ...
            .filter(Boolean)
        : undefined;
    const result = await wishlistService.listItems(userId, { page, limit }, { search, store, tags: tagList, priority, sort, order });
...
wishlist.get('/images/presign', async (c) => {
    ...
    if (!result.ok) {
        ...
        const status = result.error === 'INVALID_EXTENSION' ||
            result.error === 'INVALID_MIME_TYPE' ||
            result.error === 'FILE_TOO_LARGE' ||
            result.error === 'FILE_TOO_SMALL'
            ? 400
            : 500;
        const message = result.error === 'INVALID_EXTENSION'
...
wishlist.get('/:id', async (c) => {
    ...
    if (!result.ok) {
        ...
        if (status === 403 || status === 404) {
            logAuthorizationFailure(userId, itemId, '/wishlist/:id', 'GET', status, result.error);
        }
        return c.json({ error: result.error }, status);
    }
    return c.json(result.data);
...
wishlist.put('/:id', async (c) => {
    ...
    if (!result.ok) {
        ...
        // Log authorization failures for audit trail (AC14)
        if (status === 403 || status === 404) {
            logAuthorizationFailure(userId, itemId, '/wishlist/:id', 'PUT', status, result.error);
        }
        return c.json({ error: result.error }, status);
    }
    return c.json(result.data);
...
wishlist.delete('/:id', async (c) => {
    ...
    if (!result.ok) {
        ...
        // Log authorization failures for audit trail (AC14)
        if (status === 403 || status === 404) {
            logAuthorizationFailure(userId, itemId, '/wishlist/:id', 'DELETE', status, result.error);
        }
        return c.json({ error: result.error }, status);
    }
...
wishlist.post('/:id/purchased', async (c) => {
    ...
    if (!result.ok) {
        ...
        // Log authorization failures for audit trail (AC14)
        if (status === 403 || status === 404) {
            logAuthorizationFailure(userId, itemId, '/wishlist/:id/purchased', 'POST', status, result.error);
        }
        return c.json({ error: result.error }, status);
    }
    return c.json(result.data, 201);
