...
describe('PartsListsService', () => {
    ...
    describe('updatePart', () => {
        ...
        it('returns PART_NOT_FOUND when part does not exist', async () => {
            ...
            const result = await service.updatePart('user-123', 'moc-123', mockPartsList.id, 'nonexistent', { quantity: 20 });
            expect(result.ok).toBe(false);
            if (!result.ok) {
