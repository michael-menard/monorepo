...
import { createMocRepository, createPartsListRepository, createPartRepository, } from './adapters/index.js';
import { CreatePartsListInputSchema, UpdatePartsListInputSchema, ListPartsListsQuerySchema, CreatePartInputSchema, UpdatePartInputSchema, BulkCreatePartsInputSchema, UpdateStatusInputSchema, } from './types.js';
// ─────────────────────────────────────────────────────────────────────────
...
partsLists.get('/mocs/:mocId/:partsListId', async (c) => {
    ...
    if (!result.ok) {
        const status = result.error === 'MOC_NOT_FOUND' || result.error === 'PARTS_LIST_NOT_FOUND'
            ? 404
...
partsLists.put('/mocs/:mocId/:partsListId/status', async (c) => {
    ...
    const result = await partsListsService.updatePartsListStatus(userId, mocId, partsListId, input.data.status);
    if (!result.ok) {
        const status = result.error === 'MOC_NOT_FOUND' || result.error === 'PARTS_LIST_NOT_FOUND'
            ? 404
            : result.error === 'FORBIDDEN'
...
partsLists.patch('/mocs/:mocId/:partsListId/parts/:partId', async (c) => {
    ...
    if (!result.ok) {
        const status = result.error === 'MOC_NOT_FOUND' ||
