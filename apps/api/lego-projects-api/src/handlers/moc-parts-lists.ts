...
export const getMocPartsLists = async (req: Request, res: Response) => {
  try {
    ...
    if (!userId) {
      return res.status(403).json(apiErrorResponse(
        403,
        'UNAUTHORIZED',
    ...
    const moc = await db.select().from(mocInstructions)
    ...
    if (moc.length === 0) {
      return res.status(404).json(apiErrorResponse(
        404,
        'NOT_FOUND',
        'MOC instruction not found or not authorized'
      ));
    }

    const partsLists = await db.select().from(mocPartsLists)
      .where(eq(mocPartsLists.mocId, mocId))
      .orderBy(asc(mocPartsLists.createdAt));

    return res.status(200).json(apiResponse(
      200,
      'Parts lists retrieved successfully',
      partsLists
    ));

  } catch (error) {
    console.error('Error getting MOC parts lists:', error);
    return res.status(500).json(apiErrorResponse(
      500,
      'INTERNAL_ERROR',
      'Failed to retrieve parts lists'
    ));
  }
};
...
export const createMocPartsList = async (req: Request, res: Response) => {
  try {
    ...
    const [newPartsList] = await db.insert(mocPartsLists)
      .values({
        mocId,
        fileId,
        title,
    ...
    return res.status(201).json(apiResponse(
      201,
      'Parts list created successfully',
      newPartsList
    ));
  ...
  try {
    ...
    if (partsList.length === 0) {
      return res.status(404).json(apiErrorResponse(
        404,
        'NOT_FOUND',
        'Parts list not found or not authorized'
      ));
    }
    ...
    const [updatedPartsList] = await db.update(mocPartsLists)
      .set(updateData)
    ...
    return res.status(200).json(apiResponse(
      200,
      'Parts list updated successfully',
      updatedPartsList
    ));
...
export const updatePartsListStatus = async (req: Request, res: Response) => {
  try {
    const userId = req.authenticatedUserId;
    const { mocId, partsListId } = req.params;
    const { built, purchased, inventoryPercentage, acquiredPartsCount, actualCost, notes } = req.body;
    ...
    return res.status(200).json(apiResponse(
      200,
      'Parts list status updated successfully',
      updatedPartsList
    ));
    ...
    return res.status(500).json(apiErrorResponse(
      500,
      'INTERNAL_ERROR',
      'Failed to update parts list status'
    ));
  }
};
...
export const deleteMocPartsList = async (req: Request, res: Response) => {
  try {
    ...
    return res.status(200).json(apiResponse(
      200,
      'Parts list deleted successfully',
      null
    ));
...
export const getUserPartsListSummary = async (req: Request, res: Response) => {
  try {
    ...
    const stats = {
      ...
      averageInventoryPercentage: summary.length > 0 
        ? (summary.reduce((sum, item) => sum + parseFloat(item.inventoryPercentage || '0'), 0) / summary.length).toFixed(2)
        : '0.00'
    };

    return res.status(200).json(apiResponse(
      200,
      'Parts list summary retrieved successfully',
      {
        statistics: stats,
    ...
    return res.status(500).json(apiErrorResponse(
      500,
      'INTERNAL_ERROR',
      'Failed to retrieve parts list summary'
    ));
  }
}; 