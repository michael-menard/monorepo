 ...
 * Usage:
 * ```typescript
 * import { uploadToS3WithRetry } from '@/core/storage/retry'
 *
...
import { ExternalServiceError } from '@/core/utils/responses'
...
export async function uploadToS3WithRetry(params: {
  ...
  try {
    ...
    throw new ExternalServiceError(
      'S3 upload failed after retries. Please use the presigned URL for client-side upload.',
      {
        service: 'S3',
        presignedUrl,
        key: params.key,
...
export async function generatePresignedUploadUrl(params: {
  ...
  const presignedUrl = await getSignedUrl(s3 as any, command, { expiresIn })
...
export async function generatePresignedDownloadUrl(params: {
  ...
  return await getSignedUrl(s3 as any, command, { expiresIn })
}
