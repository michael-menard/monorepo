 ...
 */
function createMigrationPool(): pg.Pool {
  const password = process.env.PIPELINE_DB_PASSWORD
  if (!password) {
    throw new Error(
      'PIPELINE_DB_PASSWORD is required. Start the test DB with: pnpm db:test:start',
    )
  }
...
async function getAppliedVersions(pool: pg.Pool): Promise<Set<number>> {
  const result = await pool.query<{ v: number }>('SELECT v FROM checkpoint_migrations ORDER BY v')
...
async function applyMigrations(): Promise<void> {
  ...
  try {
    console.log(
      `[pipeline:migrate] Connecting to ${process.env.PIPELINE_DB_HOST || 'localhost'}:${process.env.PIPELINE_DB_PORT || '5434'}/${process.env.PIPELINE_DB_NAME || 'pipeline_test'}...`,
    )
    ...
    console.log(
      `[pipeline:migrate] Already applied versions: ${[...applied].join(', ') || 'none'}`,
    )
