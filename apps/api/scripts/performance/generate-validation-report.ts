...
function generateReport(resultsDir: string, outputPath: string): void {
  ...
  const allPassed =
    performancePassed &&
    (costPassed === null || costPassed) &&
    (coldStartPassed === null || coldStartPassed)

  markdown += '## Executive Summary\n\n'
  markdown += allPassed
    ? '✅ **VALIDATION PASSED**: All performance and cost targets met.\n\n'
  ...
  if (coldStart) {
    ...
    markdown +=
      coldStart.summary.functionsFailing === 0
  ...
  if (
    ...
    recommendations.push(
      '✅ **Proceed with ECS Decommission**: All performance and cost targets met.',
    )
  }
  ...
  if (coldStart && coldStart.summary.maxColdStartP99 > 1500) {
    recommendations.push(
      '🥶 Consider provisioned concurrency for functions with high cold start times',
    )
  }
  ...
  console.log(
    `   Cost:         ${costPassed === null ? '⚠️  SKIPPED' : costPassed ? '✅ PASSED' : '❌ FAILED'}`,
  )
  console.log(
    `   Cold Starts:  ${coldStartPassed === null ? '⚠️  SKIPPED' : coldStartPassed ? '✅ PASSED' : '❌ FAILED'}`,
  )
  console.log('')
}
...
const outputPath = args[1] ? path.resolve(args[1]) : path.join(resultsDir, 'validation-report.md')
