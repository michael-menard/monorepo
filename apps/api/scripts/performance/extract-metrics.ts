...
function extractMetrics(reportPath: string, outputPath: string): void {
  ...
  console.log(
    `Successful Requests: ${metrics.successfulRequests.toLocaleString()} (${metrics.successRate.toFixed(2)}%)`,
  )
  console.log(
    `Failed Requests:     ${metrics.failedRequests.toLocaleString()} (${metrics.errorRate.toFixed(2)}%)`,
  )
