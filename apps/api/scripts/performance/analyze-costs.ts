...
async function analyzeCosts(days = 7, baselinePath?: string): Promise<void> {
  ...
  // Load baseline
  const defaultBaselinePath = path.join(__dirname, '../../baselines/ecs-baseline.json')
  ...
  console.log(
    `   Difference:          ${savings >= 0 ? '✅ -' : '⚠️  +'}$${Math.abs(savings).toFixed(2)}/month (${savings >= 0 ? '-' : '+'}${Math.abs(savingsPercent).toFixed(1)}%)`,
  )
