...
function compareBaselines(currentPath: string, baselinePath: string): void {
  ...
  // Environment info
  console.log('\n📋 Test Information:')
  console.log(`   Baseline:  ${baseline.environment} (${new Date(baseline.testDate).toLocaleDateString()})`)
  console.log(`   Current:   ${current.environment} (${new Date(current.testDate).toLocaleDateString()})`)
  console.log('')
...
if (args.length < 2) {
  ...
  console.error(
    'Example: compare-baselines.ts results/metrics.json baselines/ecs-baseline.json'
  )
  process.exit(1)
}
