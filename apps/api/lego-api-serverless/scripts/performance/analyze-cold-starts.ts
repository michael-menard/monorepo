...
async function analyzeLambdaMetrics(
  ...
  try {
    ...
    const totalInvocations = invocationsDatapoints.reduce(
      (sum, dp) => sum + (dp.Sum || 0),
      0
    )
...
async function analyzeColdStarts(hours = 24, functionPrefix = 'lego-api'): Promise<void> {
  ...
  results.forEach((result) => {
    ...
    const coldStartStr =
      result.coldStartP99 > 0 ? `${result.coldStartP99.toFixed(0)} ms` : 'N/A'
    const warmStartStr =
      result.warmStartP99 > 0 ? `${result.warmStartP99.toFixed(0)} ms` : 'N/A'
  ...
  const maxColdStartP99 =
    functionsWithData.length > 0
      ? Math.max(...functionsWithData.map((r) => r.coldStartP99))
      : 0
  ...
  const outputPath = path.join(
    process.cwd(),
    'tests/performance/results/cold-start-analysis.json'
  )
