...
export const handler = async (event: APIGatewayProxyEventV2): Promise<APIGatewayProxyResultV2> => {
  try {
    ...
    if (!moc) {
      ...
      return createErrorResponse(404, 'NOT_FOUND', 'MOC not found or you do not have permission')
    }
    ...
    if (!image) {
      return createErrorResponse(404, 'NOT_FOUND', 'Gallery image not found')
    }
    ...
    if (existingLink) {
      return createErrorResponse(409, 'CONFLICT', 'Image is already linked to this MOC')
    }
    ...
    return createErrorResponse(500, 'INTERNAL_ERROR', 'Failed to link gallery image')
  }
}
