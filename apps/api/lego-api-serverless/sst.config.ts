...
export default $config({
  ...
  async run() {
            ...
            corsRules: [
              {
                ...
                allowedOrigins: stage === "production"
    ...
    return {
      // VPC Infrastructure
      vpc: vpc.id,
      vpcPrivateSubnets: vpc.privateSubnets.apply(subnets =>
        subnets.map(s => s.subnet.id)
      ),
      vpcPublicSubnets: vpc.publicSubnets.apply(subnets =>
        subnets.map(s => s.subnet.id)
      ),
