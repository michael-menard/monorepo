 ...
 * Usage:
 * ```typescript
 * import { traceAsyncOperation, addAnnotation, addMetadata } from '@/lib/utils/xray'
 *
 ...
 * ```
 */
export async function traceNestedOperations<T extends Record<string, (subsegment?: any) => Promise<any>>>(
  name: string,
  operations: T,
): Promise<{ [K in keyof T]: Awaited<ReturnType<T[K]>> }> {
  return await traceAsyncOperation(name, async (parentSubsegment) => {
    const results: any = {}
 ...
 */
export async function traceS3<T>(operationName: string, operation: (subsegment?: any) => Promise<T>): Promise<T> {
 ...
 * import { instrumentAWSClient } from '@/lib/utils/xray'
 *
 ...
 */
export async function traceCache<T>(operationName: string, operation: (subsegment?: any) => Promise<T>): Promise<T> {
