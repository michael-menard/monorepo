...
export const handler = async (event: APIGatewayProxyEventV2): Promise<APIGatewayProxyResultV2> => {
  try {
    ...
    if (!bucket) {
      ...
      return errorResponse(500, 'INTERNAL_ERROR', 'Server configuration error')
    }
    ...
    try {
      ...
      return errorResponse(500, 'INTERNAL_ERROR', 'Failed to download CSV file from storage')
    }
    ...
    try {
      await db.transaction(async tx => {
        ...
        for (let i = 0; i < partRecords.length; i += 1000) {
          ...
          logger.debug('Inserted parts batch', {
