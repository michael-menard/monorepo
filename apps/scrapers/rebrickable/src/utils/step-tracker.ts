...
export class StepTracker {
  ...
  /** Emit step_plan event declaring expected steps for this run. */
  async plan(steps: StepDefinition[]): Promise<void> {
    ...
    await Promise.all([
      this.emitNotification({
        type: 'step_plan',
        title: `MOC-${this.opts.mocNumber} scrape started`,
        data,
      }),
      this.writeDb({
        eventType: 'step_plan',
  ...
  /** Mark a step as completed with optional detail metadata. */
