...
import {BACKEND_URLS, createAuthTestUtils, TEST_USER} from '../auth/utils'
...
test.describe('App Pages - Native Backend Integration', () => {
  ...
  test('should handle API calls to LEGO Projects backend', async ({ page }) => {
    ...
    if (interceptData.apiRequests.length > 0) {
      const allRequestsToNativeAPI = interceptData.apiRequests.every(
        req => req.url.includes(BACKEND_URLS.api) || req.url.includes(BACKEND_URLS.auth),
      )
  ...
  test('should test error boundaries and error handling', async ({ page }) => {
    ...
    const isRedirected =
      page.url().includes('/') && !page.url().includes('/this-route-does-not-exist')
  ...
  test('should verify all backend services are correctly proxied', async ({ page }) => {
    ...
    console.log(
      `Total intercepted requests: ${interceptData.authRequests.length + interceptData.apiRequests.length}`,
    )
