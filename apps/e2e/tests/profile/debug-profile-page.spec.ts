...
test.describe('Debug Profile Page', () => {
  test('should debug profile page elements', async ({ page }) => {
    ...
    const avatarElements = await page.locator('[class*="avatar"], [data-testid*="avatar"], img[alt*="avatar"]').count();
    console.log(`👤 Avatar-related elements: ${avatarElements}`);
    
    // Look for profile-related elements
    const profileElements = await page.locator('[class*="profile"], [data-testid*="profile"]').count();
    ...
    const uploadElements = await page.locator('[class*="upload"], [data-testid*="upload"], button:has-text("Upload")').count();
