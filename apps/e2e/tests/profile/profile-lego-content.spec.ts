  ...
  test.describe('LEGO Workshop Welcome', () => {
    test('should display LEGO workshop welcome message', async ({ page }) => {
      ...
      if (await welcomeHeader.isVisible()) {
        ...
        const welcomePatterns = page.locator(
          'text=/Welcome.*LEGO|LEGO.*Workshop|Builder.*Workshop/i',
        )
  ...
  test.describe('LEGO-Themed Tabs', () => {
    ...
    test('should navigate between tabs correctly', async ({ page }) => {
      ...
      if (tabCount > 1) {
        ...
        // Look for alternative tab patterns
        const altTabs = page.locator(
          'button:has-text("MOCs"), button:has-text("Instructions"), button:has-text("Favorites")',
        )
  ...
  test.describe('MOCs Tab Content', () => {
    ...
    test('should show MOC statistics and actions', async ({ page }) => {
      ...
      if (await mocsTab.isVisible()) {
        ...
        const statsFound =
          (await downloadCounts.count()) + (await ratings.count()) + (await viewCounts.count())
        ...
        const actionButtons = page.locator(
          'button:has-text("Download"), button:has-text("View"), button:has-text("Edit")',
        )
  ...
  test.describe('Instructions Tab Content', () => {
    test('should display building instructions', async ({ page }) => {
      ...
      if (await instructionsTab.isVisible()) {
        ...
        const instructionElements = page.locator(
          'text=/Step|Guide|Tutorial|Instructions|Building/i',
        )
        ...
        if (instructionCount > 0) {
          ...
          // Check for empty state
          const emptyInstructions = page.locator(
            'text=/No instructions|Upload instructions|Create guide/i',
          )
  ...
  test.describe('Favorites Tab Content', () => {
    test('should display favorite LEGO sets and MOCs', async ({ page }) => {
      ...
      if (await favoritesTab.isVisible()) {
        ...
        const favoriteElements = page.locator(
          'text=/Favorite|Liked|Bookmarked|LEGO Creator|LEGO Technic/i',
        )
    ...
    test('should display LEGO building achievements', async ({ page }) => {
      ...
      if (await achievementsTab.isVisible()) {
        ...
        const achievementElements = page.locator(
          'text=/Achievement|Badge|Milestone|Trophy|Completed/i',
        )
        ...
        if (achievementCount > 0) {
          ...
          const emptyAchievements = page.locator(
            'text=/No achievements|Earn your first|Start building/i',
          )
