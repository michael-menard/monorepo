...
test.describe('Real E2E Auth Flow Tests (No Mocking)', () => {
  ...
  test.describe('Real CSRF Protection', () => {
    test('should fetch real CSRF token from auth service', async ({ page }) => {
      ...
      const csrfRequests = apiMonitor.authRequests.filter(req =>
        req.url.includes('/csrf') || req.url.includes('/api/auth/csrf')
      );
  ...
  test.describe('Real Session Management', () => {
    test('should make real auth check requests', async ({ page }) => {
      ...
      const authCheckRequests = apiMonitor.authRequests.filter(req =>
        req.url.includes('/check-auth') || req.url.includes('/verify') || req.url.includes('/api/auth/')
      );
