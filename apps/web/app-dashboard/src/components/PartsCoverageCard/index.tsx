/**
 * Parts Coverage Card Component
 * Shows parts inventory status with progress bars
 */

import { Check, Clock, X } from 'lucide-react'
import { Card, CardContent, CardHeader, CardTitle } from '@repo/app-component-library'
import type { PartsCoverage } from '../../__types__'
  ...
  const statuses = [
    {
      ...
      percentage: total > 0 ? Math.round((data.fullInventory / total) * 100) : 0,
      icon: Check,
      color: 'bg-[#2D5F4F] dark:bg-emerald-500',
      iconBg: 'bg-[#2D5F4F]/10 text-[#2D5F4F] dark:bg-emerald-500/10 dark:text-emerald-500',
    },
    {
      label: 'Partial - Ordered',
      count: data.partialOrdered,
      percentage: total > 0 ? Math.round((data.partialOrdered / total) * 100) : 0,
      icon: Clock,
      color: 'bg-[#D4A574] dark:bg-amber-500',
      iconBg: 'bg-[#D4A574]/10 text-[#D4A574] dark:bg-amber-500/10 dark:text-amber-500',
    },
    {
      label: 'Missing Parts',
      count: data.missingParts,
      percentage: total > 0 ? Math.round((data.missingParts / total) * 100) : 0,
      icon: X,
      color: 'bg-[#A85B4B] dark:bg-red-500',
      iconBg: 'bg-[#A85B4B]/10 text-[#A85B4B] dark:bg-red-500/10 dark:text-red-500',
    },
  ]
          ...
          <div key={status.label} className="space-y-1.5 md:space-y-2">
            <div className="flex items-center justify-between gap-2">
              <div className="flex items-center gap-1.5 md:gap-2 min-w-0">
                <div
                  className={`flex h-5 w-5 md:h-6 md:w-6 items-center justify-center rounded-full shrink-0 ${status.iconBg}`}
                >
                  <status.icon className="h-3 w-3 md:h-3.5 md:w-3.5" aria-hidden="true" />
                </div>
                <span className="text-xs md:text-sm font-medium text-card-foreground truncate">
                  {status.label}
                </span>
              </div>
