...
Given('I focus on the gallery container', async ({ page }) => {
  const gallery = page.locator(
    '[role="list"][aria-label*="Wishlist"], [data-testid="wishlist-gallery"]',
  )
  await gallery.focus()
})
...
Then('a wishlist card below should be focused', async ({ page }) => {
  const focused = page.locator(
    '[data-testid^="wishlist-card-"]:focus, [data-testid^="sortable-wishlist-card-"]:focus',
  )
...
Then(
  'the focused index should increase by approximately {int}',
  async ({ page }, approxCount: number) => {
...
Then('only one wishlist card should have tabindex {string}', async ({ page }, tabindex: string) => {
  const cardsWithTabindex = page.locator(
    `[data-testid^="wishlist-card-"][tabindex="${tabindex}"], [data-testid^="sortable-wishlist-card-"][tabindex="${tabindex}"]`,
  )
...
Then('the focused card should have a visible focus ring', async ({ page }) => {
  ...
  const hasFocusRing =
    styles.outlineWidth !== '0px' || (styles.boxShadow && styles.boxShadow !== 'none')
