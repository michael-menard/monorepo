...
When(
  'I create a wishlist item with title of {int} characters',
...
When(
  'I create a wishlist item with notes of {int} characters',
  async ({ request }, length: number) => {
    const baseUrl = process.env.API_BASE_URL || 'http://localhost:3001'
...
When(
  'I create a wishlist item with store of {int} characters',
  async ({ request }, length: number) => {
    const baseUrl = process.env.API_BASE_URL || 'http://localhost:3001'
...
When(
  'I create a wishlist item with pieceCount {float}',
  async ({ request }, pieceCount: number) => {
    const baseUrl = process.env.API_BASE_URL || 'http://localhost:3001'
...
When(
  'I mark the item as purchased with shipping {string}',
  async ({ request }, shipping: string) => {
    const baseUrl = process.env.API_BASE_URL || 'http://localhost:3001'
...
Then('the validation error should reference {string}', async ({}, field: string) => {
  const body = apiState.lastResponseBody as {
    error: string
