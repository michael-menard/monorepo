...
vi.mock('@repo/ui/select', () => ({
    __esModule: true,
    Select: ({ value, onValueChange, name, children, ...props }) => (_jsxs("select", { "aria-label": "Category", value: value, name: name, onChange: e => {
            onValueChange(e.target.value);
            // Trigger blur for validation
            e.target.blur();
        }, ...props, children: [_jsx("option", { value: "", children: "Select category" }), _jsx("option", { value: "Star Wars", children: "Star Wars" }), _jsx("option", { value: "City", children: "City" }), _jsx("option", { value: "Technic", children: "Technic" }), _jsx("option", { value: "Friends", children: "Friends" }), _jsx("option", { value: "Ninjago", children: "Ninjago" }), _jsx("option", { value: "Other", children: "Other" })] })),
    ...
    SelectValue: ({ children }) => _jsx(_Fragment, { children: children }),
...
vi.mock('../../../ui/src/FileUpload/index.js', () => ({
    FileUpload: ({ onUpload }) => (_jsx("button", { onClick: () => onUpload(new File(['img'], 'test.jpg', { type: 'image/jpeg' })), "data-testid": "mock-upload", children: "Upload" })),
}));
...
describe('AddEditWishlistModal', () => {
    it('renders in add mode with empty fields', () => {
        render(_jsx(AddEditWishlistModal, { ...defaultProps, isEdit: false }));
    ...
    it('renders in edit mode with initial values', () => {
        render(_jsx(AddEditWishlistModal, { ...defaultProps, isEdit: true, initialValues: {
                title: 'Falcon',
    ...
    it.skip('calls onSubmit with form values and closes on submit', async () => {
        ...
        render(_jsx(AddEditWishlistModal, { ...defaultProps, onSubmit: onSubmit, isEdit: false }));
        fireEvent.change(screen.getByLabelText('Title'), { target: { value: 'Falcon' } });
        fireEvent.change(screen.getByLabelText('Description'), { target: { value: 'A cool set' } });
        fireEvent.change(screen.getByLabelText('Product Link'), { target: { value: 'https://lego.com/falcon' } });
        // Open the category dropdown and select 'Star Wars'
    ...
    it('calls onClose when cancel is clicked', () => {
        const onClose = vi.fn();
        render(_jsx(AddEditWishlistModal, { ...defaultProps, onClose: onClose }));
        fireEvent.click(screen.getByText('Cancel'));
        expect(onClose).toHaveBeenCalled();
    });
    it('shows image preview after file upload', async () => {
        render(_jsx(AddEditWishlistModal, { ...defaultProps }));
