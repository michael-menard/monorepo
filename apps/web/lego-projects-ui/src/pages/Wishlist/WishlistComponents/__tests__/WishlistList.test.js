...
describe('WishlistList', () => {
    ...
    it('calls onEdit and onDelete when buttons are clicked', () => {
        ...
        render(_jsx(WishlistList, { items: mockItems, onEdit: onEdit, onDelete: onDelete, persistKey: persistKey }));
    ...
    it('handles empty list gracefully', () => {
        render(_jsx(WishlistList, { items: [], persistKey: persistKey }));
        expect(screen.queryAllByRole('button').length).toBe(0);
    });
    it('handles single-item list', () => {
        render(_jsx(WishlistList, { items: [mockItems[0]], persistKey: persistKey }));
        expect(screen.getByText('Falcon')).toBeInTheDocument();
    });
