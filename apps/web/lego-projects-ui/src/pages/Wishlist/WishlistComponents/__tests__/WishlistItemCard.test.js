...
describe('WishlistItemCard', () => {
    it('renders all item details', () => {
        render(_jsx(WishlistItemCard, { item: mockItem, onEdit: vi.fn(), onDelete: vi.fn() }));
    ...
    it('calls onEdit when edit button is clicked', () => {
        const onEdit = vi.fn();
        render(_jsx(WishlistItemCard, { item: mockItem, onEdit: onEdit, onDelete: vi.fn() }));
        fireEvent.click(screen.getByLabelText('Edit item'));
        expect(onEdit).toHaveBeenCalledWith('item1');
    });
    it('calls onDelete when delete button is clicked', () => {
        const onDelete = vi.fn();
        render(_jsx(WishlistItemCard, { item: mockItem, onEdit: vi.fn(), onDelete: onDelete }));
    ...
    it('calls onDragStart and onDragEnd when dragged', () => {
        ...
        render(_jsx(WishlistItemCard, { item: mockItem, onEdit: vi.fn(), onDelete: vi.fn(), onDragStart: onDragStart, onDragEnd: onDragEnd }));
    ...
    it('renders without image or product link if not provided', () => {
        ...
        render(_jsx(WishlistItemCard, { item: item, onEdit: vi.fn(), onDelete: vi.fn() }));
