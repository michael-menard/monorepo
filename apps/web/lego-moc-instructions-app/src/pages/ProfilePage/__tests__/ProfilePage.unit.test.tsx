  ...
  ProfileSidebar: ({ profile, onEdit, onUploadAvatar }: any) => (
    <div data-testid="profile-sidebar">
      ...
      <div className="space-y-2">
        <button onClick={onEdit} data-testid="sidebar-edit-btn">
          Edit Profile
        </button>
        <button
          onClick={() => onUploadAvatar(new File([''], 'test.jpg', { type: 'image/jpeg' }))}
  ...
  return {
    ...
    AppAvatar: ({
      ...
      <div className={`relative inline-block ${className || ''}`}>
        ...
        >
          <div
            className={`rounded-full overflow-hidden ${size === 'sm' ? 'w-8 h-8' : size === 'lg' ? 'w-12 h-12' : 'w-10 h-10'}`}
          >
            <img
              src={avatarUrl}
    ...
    it('should populate form fields with current profile data', async () => {
      ...
      await waitFor(() => {
        ...
        expect(screen.getByTestId('input-bio')).toHaveValue(
          'LEGO enthusiast and MOC creator. I love building custom models and sharing instructions with the community.',
        )
