...
vi.mock('@repo/profile', () => ({
  ...
  ProfileAvatar: ({
    ...
    <div data-testid="profile-avatar" data-size={size} data-editable={editable}>
      ...
      {showVerified ? (
        <div data-testid="avatar-verified" data-verified={isVerified}>
          Verified
        </div>
      ) : null}
      {editable ? (
...
vi.mock('@repo/profile', () => ({
  ...
  AvatarUploader: ({ currentAvatar, onUpload }: any) => (
    <div data-testid="avatar-uploader">
      ...
      <button data-testid="upload-button" onClick={() => onUpload?.(new File([''], 'test.jpg'))}>
        Upload New Avatar
      </button>
    </div>
  ),
...
describe('ProfilePage - New RTK Layout', () => {
  ...
  describe('Main Content', () => {
    it('renders ProfileMain with correct title and description', () => {
      ...
      expect(screen.getByTestId('main-description')).toHaveTextContent(
  ...
  describe('Edit Profile Modal', () => {
    it('opens edit modal when edit button is clicked', async () => {
      ...
      const editButtons = screen.getAllByText('Edit Profile')
      const sidebarEditButton = editButtons.find(
        button => button.tagName === 'BUTTON' || button.closest('button'),
      )
    ...
    it('closes modal when cancel button is clicked', async () => {
      ...
      if (sidebarEditButton) {
        ...
        await waitFor(() => {
          ...
          expect(
            screen.queryByRole('heading', { level: 2, name: 'Edit Profile' }),
