  ...
  ProfileAvatar: ({
    ...
    <div data-testid="profile-avatar" className={className} data-size={size}>
      <div data-testid="avatar-container" className="relative group">
        <img src={avatarUrl} alt={`${userName}'s avatar`} data-testid="avatar-image" />
        ...
        {showVerified && isVerified ? <div data-testid="avatar-verified">✓</div> : null}
      </div>
      <input type="file" accept="image/*" className="hidden" data-testid="avatar-file-input" />
    </div>
  ),
  ProfileAvatarInfo: ({
    userName,
    ...
    <div data-testid="profile-avatar-info">
      ...
      {username ? <div data-testid="username">@{username}</div> : null}
      {title ? <div data-testid="user-title">{title}</div> : null}
      {location ? <div data-testid="user-location">{location}</div> : null}
      {joinDate ? <div data-testid="join-date">{joinDate.toLocaleDateString()}</div> : null}
      {badges
...
describe('ProfilePage with New Layout System', () => {
  ...
  describe('LEGO-Themed Content', () => {
    it('should render LegoProfileContent component', () => {
      ...
      expect(screen.getByTestId('welcome-header')).toHaveTextContent(
        "Welcome to John's LEGO Workshop!",
      )
    })
