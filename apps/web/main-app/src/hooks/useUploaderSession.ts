...
export function useUploaderSession(options: UseUploaderSessionOptions): UseUploaderSessionResult {
  ...
  }, [route, isAuthenticated, user?.id, getAnonSessionId])
  ...
  useEffect(() => {
    try {
      ...
      if (parsed) {
        ...
        if (isAuthenticated && parsed.anonSessionId && !user?.id) {
  ...
  const reset = useCallback(() => {
    ...
    try {
      localStorage.removeItem(storageKey)
      logger.info('Uploader session reset', { route, userId: user?.id })
    } catch (error) {
  ...
  const markFinalized = useCallback(() => {
    reset()
    logger.info('Uploader session finalized', { route, userId: user?.id })
  }, [reset, route, user?.id])
