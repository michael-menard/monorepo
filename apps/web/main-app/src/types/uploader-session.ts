/**
 * Story 3.1.9: Uploader Session Types
 *
 ...
 */
export function getStorageKey(route: string, userId?: string, anonSessionId?: string): string {
 ...
 */
export function parseSession(json: string | null): UploaderSession | null {
  if (!json) return null
 ...
 */
export function serializeSession(session: UploaderSession): string {
  const withTimestamp = {
    ...session,
    updatedAt: Date.now(),
  }
 ...
 */
export function createEmptySession(): UploaderSession {
  return UploaderSessionSchema.parse({})
}

/**
 * Extract file metadata from File object (no blob).
 */
export function fileToMetadata(file: File, fileType?: FileMetadata['fileType']): FileMetadata {
  return {
    name: file.name,
    size: file.size,
    type: file.type,
 ...
 */
export function migrateSession(session: UploaderSession): UploaderSession {
