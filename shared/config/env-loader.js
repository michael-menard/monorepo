...
const envMappings = {
  // Map centralized ports to service-specific PORT variables
  PORT: process.env.LEGO_API_PORT || process.env.AUTH_SERVICE_PORT || process.env.PORT,
...
const detectServicePort = () => {
  ...
  if (cwd.includes('auth-service')) {
    process.env.PORT = process.env.AUTH_SERVICE_PORT || process.env.PORT || '9300';
    console.log(`🔧 Auth Service detected - using port ${process.env.PORT}`);
  } else if (cwd.includes('lego-projects-api')) {
    process.env.PORT = process.env.LEGO_API_PORT || process.env.PORT || '9000';
...
module.exports = {
  ...
  urls: {
    auth: process.env.AUTH_SERVICE_URL || `http://localhost:${process.env.AUTH_SERVICE_PORT || '9300'}`,
