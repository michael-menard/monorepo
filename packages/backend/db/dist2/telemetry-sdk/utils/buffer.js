 ...
 */
export function addEventToBuffer(state, event, config) {
    const bufferedEvent = {
        event,
        timestamp: new Date(),
    };
    ...
    if (state.events.length >= config.bufferSize) {
        switch (config.overflowStrategy) {
            case 'drop-oldest':
                ...
                break;
            case 'error':
                throw new Error(`[telemetry-sdk] Buffer overflow: buffer size limit ${config.bufferSize} reached`);
            case 'block':
 ...
 */
export function getEventsToFlush(state) {
    if (state.isFlushing) {
 ...
 */
export function markFlushComplete(state) {
    return {
        ...state,
        isFlushing: false,
    };
 ...
 */
export function shouldFlushBySize(state, config) {
 ...
 */
export function shouldFlushByInterval(state, config) {
 ...
 */
export function hasEventsToFlush(state) {
