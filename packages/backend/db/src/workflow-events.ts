 ...
 */
export async function insertWorkflowEvent(event: WorkflowEventInput): Promise<void> {
  ...
  if (parsed.payload !== null && parsed.payload !== undefined) {
    ...
    try {
      ...
      throw new Error(
        `[telemetry] Invalid payload for event_type '${parsed.eventType}': ${error instanceof Error ? error.message : String(error)}`,
      )
    }
  }
  ...
  try {
    ...
    logger.warn(
      `[telemetry] Failed to insert workflow event ${parsed.eventId}:`,
