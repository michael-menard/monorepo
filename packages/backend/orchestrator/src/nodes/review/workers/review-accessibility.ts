/**
 * Review Accessibility Worker
 *
 * LLM-powered review of accessibility patterns (ARIA, keyboard nav, focus management).
...
export const ReviewAccessibilityConfigSchema = z.object({
  timeoutMs: z.number().int().positive().default(60000),
  enabled: z.boolean().default(true),
  modelRouterOverride: z.function().optional(),
})

export type ReviewAccessibilityConfig = z.infer<typeof ReviewAccessibilityConfigSchema>
...
export function createReviewAccessibilityNode(
  ...
  return async (state: {
    ...
    try {
      const prompt = buildAccessibilityReviewPrompt(state.changeSpecIds ?? [])
...
function buildAccessibilityReviewPrompt(changeSpecIds: string[]): string {
  const fileList = changeSpecIds.length > 0 ? changeSpecIds.join('\n') : 'all changed files'
  return `Review the following files for accessibility compliance:
${fileList}
