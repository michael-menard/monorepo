...
2. `@repo/orchestrator` builds second (re-exports schemas)
...
- **Never** import `orchestrator` in `database-schema`
...
4. Rebuild orchestrator: `pnpm --filter @repo/orchestrator build`
...
```typescript
// In this file (orchestrator/src/__types__/index.ts)
