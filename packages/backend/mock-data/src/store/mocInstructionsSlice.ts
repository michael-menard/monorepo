  ...
  async (filters: Partial<MocInstructionsState['filters']> = {}, { rejectWithValue }) => {
    try {
      ...
      if (filters) {
        ...
        if (filters.category) {
          filteredInstructions = filteredInstructions.filter(
            inst => inst.category === filters.category,
          )
        }

        if (filters.difficulty) {
          filteredInstructions = filteredInstructions.filter(
            inst => inst.difficulty === filters.difficulty,
          )
        }
        ...
        if (filters.minParts) {
          filteredInstructions = filteredInstructions.filter(
            inst => (inst.totalParts || 0) >= filters.minParts!,
          )
        }

        if (filters.maxParts) {
          filteredInstructions = filteredInstructions.filter(
            inst => (inst.totalParts || 0) <= filters.maxParts!,
          )
        }

        if (filters.minTime) {
          filteredInstructions = filteredInstructions.filter(
            inst => (inst.estimatedTime || 0) >= filters.minTime!,
          )
        }

        if (filters.maxTime) {
          filteredInstructions = filteredInstructions.filter(
            inst => (inst.estimatedTime || 0) <= filters.maxTime!,
          )
        }

        if (filters.isPublic !== null && filters.isPublic !== undefined) {
          filteredInstructions = filteredInstructions.filter(
            inst => inst.isPublic === filters.isPublic,
          )
        }

        if (filters.isPublished !== null && filters.isPublished !== undefined) {
          filteredInstructions = filteredInstructions.filter(
            inst => inst.isPublished === filters.isPublished,
          )
        }
...
export const createMocInstruction = createAsyncThunk(
  ...
  async (
    instruction: Omit<
      MockInstruction,
      'id' | 'createdAt' | 'updatedAt' | 'downloadCount' | 'rating'
    >,
    { rejectWithValue },
  ) => {
    try {
...
export const updateMocInstruction = createAsyncThunk(
  ...
  async (
    { id, updates }: { id: string; updates: Partial<MockInstruction> },
    { rejectWithValue },
  ) => {
    try {
      // Simulate API call
...
export const { setFilter, clearFilters, clearError, setSelectedInstruction, updateStats } =
  mocInstructionsSlice.actions

export default mocInstructionsSlice.reducer

// Selectors
export const selectMocInstructions = (state: { mocInstructions: MocInstructionsState }) =>
  state.mocInstructions.instructions
export const selectMocInstructionsLoading = (state: { mocInstructions: MocInstructionsState }) =>
  state.mocInstructions.loading
export const selectMocInstructionsError = (state: { mocInstructions: MocInstructionsState }) =>
  state.mocInstructions.error
export const selectMocInstructionsFilters = (state: { mocInstructions: MocInstructionsState }) =>
  state.mocInstructions.filters
...
export const selectSelectedMocInstruction = (state: { mocInstructions: MocInstructionsState }) =>
  state.mocInstructions.selectedInstruction
...
export const selectFilteredMocInstructions = (state: { mocInstructions: MocInstructionsState }) => {
  ...
  if (filters.tags.length > 0) {
    filtered = filtered.filter(inst => filters.tags.some(tag => inst.tags.includes(tag)))
  }
...
export const selectMocInstructionCategories = (state: {
  mocInstructions: MocInstructionsState
}) => {
