...
export const UploadArea = React.forwardRef<HTMLDivElement, UploadAreaProps>(
  ({ upload, config, preset, disabled = false, variant = 'default', className, ...props }, ref) => {
    ...
    const dragClasses = dragAndDrop.isDragActive ? 'border-blue-500 bg-blue-50' : ''
    ...
    return (
      ...
      >
        ...
        {variant === 'avatar' ? (
          <div className="text-xs text-gray-500">{upload.files.length > 0 ? '✓' : '+'}</div>
        ) : (
