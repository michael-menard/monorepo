...
import { fileFixtures, configFixtures, serverResponseFixtures } from './fixtures.js'
import { mockXMLHttpRequest, simulateUploadError, mockImageProcessing } from './mocks.js'
...
describe('error scenarios and edge cases', () => {
  ...
  describe('file validation error scenarios', () => {
    ...
    it('should handle oversized files', () => {
      const oversizedFiles = [fileFixtures.largeImage]
  ...
  describe('image processing error scenarios', () => {
    ...
    it('should handle canvas creation failures', async () => {
      ...
      await expect(compressImage(fileFixtures.jpegImage)).rejects.toThrow('Canvas not supported')
    })
  ...
  describe('hook error scenarios', () => {
    ...
    it('should handle malformed file objects', () => {
      ...
      expect(() => {
        validateFiles(
          malformedFiles.filter(f => f != null),
          configFixtures.default,
        )
      }).not.toThrow()
    })
  ...
  describe('concurrent operation errors', () => {
    it('should handle multiple simultaneous uploads', async () => {
      ...
      const uploadPromises = files.map(file => uploadSingleFile(file, config))
