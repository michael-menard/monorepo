...
describe('CreateAlbumDialog', () => {
  beforeEach(async () => {
    ...
    const { useCreateAlbumMutation, useAddImageToAlbumMutation } = vi.mocked(
      await import('../../../store/albumsApi.js'),
    )
    const mockCreateAlbum = vi.fn().mockResolvedValue({ album: { id: 'album-123' } })
    const mockAddImageToAlbum = vi.fn().mockResolvedValue({})

    useCreateAlbumMutation.mockReturnValue([
      mockCreateAlbum,
      { isLoading: false, error: null, reset: vi.fn() },
    ])
    useAddImageToAlbumMutation.mockReturnValue([
      mockAddImageToAlbum,
      { isLoading: false, error: null, reset: vi.fn() },
    ])
  })
