...
export const DownloadManager: React.FC<DownloadManagerProps> = ({
  ...
  const updateDownloadProgress = useCallback((id: string, progress: DownloadProgress) => {
    setDownloads(prev =>
      prev.map(download => (download.id === id ? { ...download, progress } : download)),
    )
  }, [])

  const updateDownloadStatus = useCallback(
    (id: string, status: DownloadItem['status'], result?: DownloadResult, error?: string) => {
      setDownloads(prev =>
        prev.map(download =>
  ...
  const cancelDownload = useCallback((id: string) => {
    setDownloads(prev =>
      prev.map(download =>
        download.id === id
          ? { ...download, status: 'error', error: 'Download cancelled' }
          : download,
      ),
    )
  }, [])
