...
export const validateImageFile = (file: File, maxSize = 10 * 1024 * 1024): { isValid: boolean; error?: string } => {
...
export const getFileTypeCategory = (file: File): 'instruction' | 'parts-list' | 'image' | 'unknown' => {
  if (isImageFile(file)) {
    return 'image';
  }
