...
describe('ProfileSidebar', () => {
  ...
  it('renders profile information correctly', () => {
    ...
    expect(
      screen.getByText(
        'Software developer with 5+ years of experience in React, TypeScript, and Node.js.',
      ),
    ).toBeInTheDocument()
  })
  ...
  it('handles avatar upload when avatar is clicked in editable mode', async () => {
    render(
      <ProfileSidebar
        profile={mockProfile}
        isEditable={true}
        onUploadAvatar={mockOnUploadAvatar}
      />,
    )
  ...
  it('applies custom className', () => {
    const { container } = render(
      <ProfileSidebar profile={mockProfile} className="custom-sidebar" />,
    )
