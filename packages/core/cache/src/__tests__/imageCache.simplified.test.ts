...
describe('ImageCache - Simplified Unit Tests', () => {
  ...
  describe('Cache Statistics', () => {
    it('should calculate cache size and entry count', () => {
      ...
      localStorage.setItem('image-cache-1', JSON.stringify({ dataUrl: 'data:image/jpeg;base64,abc', timestamp: Date.now() }));
      localStorage.setItem('image-cache-2', JSON.stringify({ dataUrl: 'data:image/png;base64,xyz', timestamp: Date.now() }));
    ...
    it('should identify expired entries for cleanup', () => {
      ...
      const now = Date.now();
      localStorage.setItem('image-cache-expired', JSON.stringify({ 
        dataUrl: 'data:image/jpeg;base64,old', 
        timestamp: now - 7200000 // 2 hours ago
      }));
      localStorage.setItem('image-cache-fresh', JSON.stringify({ 
