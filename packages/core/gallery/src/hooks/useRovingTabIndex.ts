...
export const RovingTabIndexOptionsSchema = z.object({
  ...
  /** Accessible label for the grid (default: 'Gallery items') */
  ariaLabel: z.string().optional().default('Gallery items'),
})
 ...
 */
export const UseRovingTabIndexReturnSchema = z.object({
  /** Currently active index */
  activeIndex: z.number(),
  /** Set the active index programmatically */
  setActiveIndex: z.custom<(index: number) => void>(),
  /** Get the tabIndex value for an item */
  getTabIndex: z.custom<(index: number) => 0 | -1>(),
  /** Check if an item is the active one */
  isActive: z.custom<(index: number) => boolean>(),
  /** Key down handler for the container */
  handleKeyDown: z.custom<(event: React.KeyboardEvent) => void>(),
  /** Props to spread on the container element */
  containerProps: z.object({
    role: z.literal('grid'),
    'aria-label': z.string(),
    onKeyDown: z.custom<(event: React.KeyboardEvent) => void>(),
  }),
  /** Props to spread on each grid item */
  getItemProps: z.custom<
    (index: number) => {
      role: 'gridcell'
      tabIndex: 0 | -1
...
export type UseRovingTabIndexReturn = z.infer<typeof UseRovingTabIndexReturnSchema>

/**
 * Calculate the number of columns based on item positions
 * This is used when columns are not explicitly provided
 *
