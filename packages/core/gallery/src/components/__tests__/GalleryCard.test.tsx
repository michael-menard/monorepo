...
describe('GalleryCard', () => {
  ...
  describe('showContent Prop', () => {
    it('shows content area by default', () => {
      render(<GalleryCard {...defaultProps} subtitle="Subtitle" />)

      expect(screen.getByTestId('gallery-card-title')).toBeInTheDocument()
      expect(screen.getByTestId('gallery-card-subtitle')).toBeInTheDocument()
    })

    it('hides content area when showContent is false', () => {
      render(<GalleryCard {...defaultProps} subtitle="Subtitle" showContent={false} />)

      expect(screen.queryByTestId('gallery-card-title')).not.toBeInTheDocument()
      expect(screen.queryByTestId('gallery-card-subtitle')).not.toBeInTheDocument()
    })

    it('still provides aria-label when showContent is false', () => {
      render(<GalleryCard {...defaultProps} showContent={false} onClick={vi.fn()} />)

      expect(screen.getByRole('button', { name: 'Test Card' })).toBeInTheDocument()
    })
  })

  describe('Optional Image & imageFallback', () => {
    it('renders placeholder when no image prop provided', () => {
      render(<GalleryCard title="No Image" />)

      expect(screen.getByTestId('gallery-card')).toBeInTheDocument()
      expect(screen.getByTestId('gallery-card-image-placeholder')).toBeInTheDocument()
    })

    it('renders imageFallback when no image provided', () => {
      render(
        <GalleryCard
          title="Fallback Card"
          imageFallback={<div data-testid="custom-fallback">Folder Icon</div>}
        />,
      )

      expect(screen.getByTestId('custom-fallback')).toBeInTheDocument()
      expect(screen.getByTestId('gallery-card-image-fallback')).toBeInTheDocument()
    })

    it('renders image normally when image prop is provided', () => {
      render(<GalleryCard {...defaultProps} />)

      expect(screen.getByTestId('gallery-card-image')).toBeInTheDocument()
      expect(screen.queryByTestId('gallery-card-image-placeholder')).not.toBeInTheDocument()
      expect(screen.queryByTestId('gallery-card-image-fallback')).not.toBeInTheDocument()
    })
  })
  ...
  describe('Image Handling', () => {
    ...
    it.skip('shows error fallback on image error', () => {
      render(<GalleryCard {...defaultProps} />)
