...
import { getAriaAttributes, useUniqueId, useFocusTrap, KEYBOARD_KEYS } from './lib/keyboard-navigation';
  ...
  (
    ...
    const handleSelectAll = () => {
      ...
      if (allSelected) {
        ...
        const newSelection = [...new Set([...selectedValues, ...selectableOptions.map((option) => option.value)])];
        onSelectionChange(newSelection);
      }
    };
    ...
    const selectableOptions = filteredOptions.filter((option) => !option.disabled);
    const allFilteredSelected = selectableOptions.length > 0 && selectableOptions.every((option) =>
      selectedValues.includes(option.value)
    );
