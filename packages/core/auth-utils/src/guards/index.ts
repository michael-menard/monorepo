...
/**
 * Route guard options schema
 */
export const RouteGuardOptionsSchema = z.object({
  ...
  checkTokenExpiry: z.boolean().optional(),
})

export type RouteGuardOptions = z.infer<typeof RouteGuardOptionsSchema>
  ...
  return ({ context, location }: { context: any; location: any }) => {
    ...
    if (options.checkTokenExpiry !== false && auth.isAuthenticated && auth.tokens?.accessToken) {
      if (isTokenExpired(auth.tokens.accessToken)) {
        ...
        throw redirectFn({
          to: options.redirectTo || '/login',
          search: { redirect: location.pathname, expired: true } as any,
        })
      }
    }
...
/**
 * Route metadata schema for navigation and breadcrumbs
 */
export const RouteMetadataSchema = z.object({
  ...
  category: z.enum(['primary', 'secondary', 'utility']).optional(),
