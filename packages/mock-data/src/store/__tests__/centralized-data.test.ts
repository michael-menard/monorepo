...
describe('Centralized Data Integration', () => {
  ...
  describe('MOC Instructions Centralization', () => {
    ...
    it('should support advanced filtering', async () => {
      ...
      expect(largeBuildsResult.payload.every((inst: any) => (inst.totalParts || 0) >= 1500)).toBe(true);
      
      // Test time range filtering
      const quickBuildsResult = await store.dispatch(fetchMocInstructions({ maxTime: 5 }));
      expect(quickBuildsResult.payload.every((inst: any) => (inst.estimatedTime || 0) <= 5)).toBe(true);
    });

    it('should support search functionality', async () => {
      const searchResult = await store.dispatch(fetchMocInstructions({ search: 'dragon' }));
      const hasSearchTerm = searchResult.payload.some((inst: any) => 
    ...
    it('should support sorting', async () => {
      ...
      const popularResult = await store.dispatch(fetchMocInstructions({ 
      ...
      const alphabeticalResult = await store.dispatch(fetchMocInstructions({ 
        sortBy: 'title', 
        sortOrder: 'asc' 
      }));
    ...
    it('should calculate accurate stats', async () => {
      ...
      expect(stats?.totalDownloads).toBe(instructions.reduce((sum, inst) => sum + inst.downloadCount, 0));
      
      const avgRating = instructions.reduce((sum, inst) => sum + (inst.rating || 0), 0) / instructions.length;
