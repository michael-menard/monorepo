...
export class ServiceClient {
  ...
  async request<T>(method: string, path: string, body?: any, options?: RequestOptions): Promise<T> {
    // Implementation details in next section
  }
 ...
 */
export function generateHMACSignature(data: HMACSignatureData, secret: string): string {
  const stringToSign = [
...
describe('HMAC Integration', () => {
  ...
  test('should reject requests without HMAC headers', async () => {
    const response = await request(app).get('/api/internal/users').expect(401)
