...
predictions:
  null # Placeholder for future calibration data
  # When implemented:
...
sources:
  token_log: string # "kb:token_usage" (KB storyTokenUsage entries)
...
1. Queries KB `storyTokenUsage` for per-phase token counts
...
5. **Reads KB story metadata to extract `experiment_variant` field** (WKFL-008)
6. Aggregates totals
7. Writes OUTCOME.yaml to `_implementation/`

**Experiment Variant Propagation** (WKFL-008):

- Read `experiment_variant` from KB story metadata (`kb_get_story`)
- Include in OUTCOME.yaml exactly as specified
- If field missing from KB metadata, write `null` (legacy story)
...
- Updated generation process to read variant from KB story metadata
