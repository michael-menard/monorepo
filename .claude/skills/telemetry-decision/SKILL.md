...
description: Log a human-in-the-loop (HiTL) decision with semantic embedding to wint.hitl_decisions. Called by operators during story review, QA gate, and sign-off phases to capture structured decisions for preference learning and semantic similarity search.
kb_tools:
...
4. Row inserted in `wint.hitl_decisions` with `decision_type=approve`, `story_id='WINT-3040'`, `embedding` populated.
...
4. Row inserted in `wint.hitl_decisions` with `decision_type=request_changes`, `embedding=NULL`.
...
FROM wint.hitl_decisions ORDER BY created_at DESC LIMIT 1;
-- expect: true
```
