...
- `EVIDENCE-JUDGE COMPLETE` or `EVIDENCE-JUDGE COMPLETE WITH WARNINGS` → Read ac-verdict artifact from KB:
  ```javascript
  const acVerdict = await kb_read_artifact({
    story_id: '{STORY_ID}',
    artifact_type: 'analysis',
    artifact_name: 'ac-verdict',
  })
  ```
  If null: log warning `"QA Advisory: ac-verdict artifact not found in KB — continuing to Phase 2"`, proceed to Phase 2.
...
| `CHALLENGE`       | Add advisory note to QA report: `"QA Advisory [WINT-4120 Phase 1.5]: One or more ACs have weak evidence. See ac-verdict KB artifact for details."`. Proceed to Phase 2.      |
| `FAIL`            | Surface as QA issue in report: `"QA Issue [WINT-4120 Phase 1.5]: One or more ACs have no verifiable evidence. See ac-verdict KB artifact for details."`. Proceed to Phase 2. |
...
| Component      | Agent file                | Model | Output                                       | Signal                                                           |
| -------------- | ------------------------- | ----- | -------------------------------------------- | ---------------------------------------------------------------- |
| evidence-judge | `evidence-judge.agent.md` | haiku | KB artifact (type=analysis, name=ac-verdict) | `EVIDENCE-JUDGE COMPLETE` / `COMPLETE WITH WARNINGS` / `BLOCKED` |
...
| ac-verdict artifact absent in KB                               | Log warning, proceed to Phase 2                    |
...
- CHALLENGE: `"QA Advisory [WINT-4120 Phase 1.5]: {details from ac-verdict KB artifact}"`
- FAIL: `"QA Issue [WINT-4120 Phase 1.5]: {details from ac-verdict KB artifact}"`

**Graceful degradation:** Stories without a KB evidence artifact (pre-Phase-4 infra or docs stories) complete normally. evidence-judge emits `COMPLETE WITH WARNINGS` with all-REJECT verdicts for missing evidence artifact, which is handled by the signal routing table above (as an advisory FAIL note).
