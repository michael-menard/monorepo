...
kb_add_lesson({
  ...
  category: 'pattern',
  what_happened:
    '{agent_id} stated {confidence} confidence on {total} findings, but only {correct} were correct ({percent}% accuracy)',
...
const entries = kb_search({
  ...
  tags: agentFilter ? ['calibration', `agent:${agentFilter}`] : ['calibration'],
...
Call `kb_write_artifact({ story_id: 'WKFL-002', artifact_type: 'evidence', artifact_name: 'CALIBRATION-REPORT', content: { ... } })` with the following structure:
...
| ------------------------------------------------------------------------------------------------------------------------------- | -------------- | ---------------------------------------------------- |
| `kb_write_artifact({ story_id: 'WKFL-002', artifact_type: 'evidence', artifact_name: 'CALIBRATION-REPORT', content: { ... } })` | Knowledge Base | Full calibration report — primary write              |
| KB alert entries                                                                                                                | Knowledge Base | Significant calibration alerts (via `kb_add_lesson`) |
...
- MUST write calibration report to KB via `kb_write_artifact({ story_id: 'WKFL-002', artifact_type: 'evidence', artifact_name: 'CALIBRATION-REPORT', content: { ... } })`
...
| Output                                                              | Consumer                                          |
| ------------------------------------------------------------------- | ------------------------------------------------- |
| KB `evidence` artifact (CALIBRATION-REPORT) via `kb_write_artifact` | Human review, heuristic-evolver (WKFL-003)        |
