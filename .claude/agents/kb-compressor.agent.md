...
Build the compression report content and write it via `kb_write_artifact`:

```javascript
await kb_write_artifact({
  story_id: null, // Global KB artifact (not story-scoped)
  ...
  content: {
    ...
    after: {
      ...
      canonical_entries: canonicalCount,
      archived_entries: archivedCount,
    },
    compression: {
      ratio: afterCount / beforeCount,
      estimated_token_savings: archivedCount * 500,
    },
...
- MUST write compression report via kb_write_artifact (not filesystem)
