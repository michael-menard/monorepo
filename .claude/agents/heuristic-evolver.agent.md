...
**KB (preferred)**: `kb_read_artifact({ story_id: 'WKFL-003', artifact_type: 'context' })` — current tier definitions and patterns

**Fallback**: `.claude/config/decision-classification.yaml`
...
1. **Read Current Tier Definitions**
   ...
   kb_read_artifact({ story_id: 'WKFL-003', artifact_type: 'context' })
   // Fallback: Read .claude/config/decision-classification.yaml
   ```
...
Call `kb_write_artifact({ story_id: 'WKFL-003', artifact_type: 'plan', artifact_name: 'HEURISTIC-PROPOSALS', content: { ... } })` with the following structure:
...
`kb_write_artifact({ story_id: 'WKFL-003', artifact_type: 'plan', artifact_name: 'HEURISTIC-PROPOSALS', content: { promotions: [...], demotions: [...], no_change_patterns: [...] } })`

Generated proposals for human review.
...
| -------------------------------------------------------------------------------- | -------------------------------------------------- |
| `.claude/config/decision-classification.yaml`                                    | Source of truth for current tier rules (read-only) |
| KB artifact: `kb_read_artifact({ story_id: 'WKFL-003', artifact_type: 'plan' })` | Output proposals in KB (this agent writes it)      |
...
| 1.2.0   | 2026-03-22 | Migrated filesystem artifact writes to KB; added kb_add_lesson to kb_tools |
