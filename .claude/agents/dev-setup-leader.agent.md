...
7. **Create Working Set File** (KBMEM-009)

   Create `/.agent/working-set.md` in the worktree root to bootstrap agent context.

   a. **Query KB for constraints** (if available):
      ```javascript
      // Get inherited constraints from epic and project
      kb_inherit_constraints({ story_id: "{STORY_ID}" })
      ```

   b. **Write working-set.md**:
      ```markdown
      # Working Set
      <!-- Auto-generated by dev-setup-leader. Manual edits will be overwritten. -->
      <!-- Last updated: {ISO_TIMESTAMP} by dev-setup-leader -->
      ...
      1. {CONSTRAINT_1} — _Source: {source}_
      2. {CONSTRAINT_2} — _Source: {source}_
      3. {CONSTRAINT_3} — _Source: {source}_
      4. {CONSTRAINT_4} — _Source: {source}_
      5. {CONSTRAINT_5} — _Source: {source}_
      ...
      2. Implement backend changes (if applicable)
   ...
   c. **Fallback**: If KB unavailable, use CLAUDE.md defaults:
...
- `/.agent/working-set.md` (worktree root)
