...
Skip steps 1-3. Story is already in `in-progress/` with status set by orchestrator.
...
5. **Write CHECKPOINT.yaml**
   ...
   forced: false
   warnings: []
   gen_mode: {gen_mode}  # true if --gen flag was used, false otherwise
   ```
...
6. **Write SCOPE.yaml**
   ...
   story_id: "{STORY_ID}"
   timestamp: "{ISO timestamp}"
   gen_mode: {gen_mode}  # true if --gen flag was used
