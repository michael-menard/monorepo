...
```javascript
kb_search({
  query: 'fix iteration lessons learned',
  tags: ['lesson-learned', 'stage:dev'],
  limit: 3,
})
```
...
**From KB** (`kb_get_story({ story_id: "{STORY_ID}" })`):
...
| No blocking dependencies (if not gen_mode) | KB story shows no blockers                                                   | STOP: "Blocked by: [list]"         |

### Actions (Sequential)

**Both modes:**

1. **Write Checkpoint Artifact (KB-only)**
...
2. **Write Scope Artifact (KB-only)**

   Analyze story keywords/title from KB record to determine scope:
...
3. **Sync Working Set to KB** (KBMEM-009)
...
4. **Update Story Status in KB** (track story progress)
   ```javascript
   kb_update_story_status({
     story_id: '{STORY_ID}',
     state: 'in_progress',
...
4. **Write Fix Summary Artifact to KB**
   ...
   kb_write_artifact({
     ...
     content: {
       ...
       focus_files: ['path/to/file1.ts', 'path/to/file2.ts'],
     },
   })
   ```
...
Call `session_create` at the very start of the workflow (before precondition checks) and `session_complete` before emitting SETUP COMPLETE. Session tracking is telemetry — null returns MUST NOT gate the setup flow.

### Session Create (both implement and fix modes)

```javascript
// At workflow start, before any precondition checks:
...
- MUST read story from KB (`kb_get_story`) — KB is the only source of truth
...
- Do NOT guess scope - analyze keywords in title/description from KB record
