...
- `kb_get_story({ story_id: "{STORY_ID}" })` — fetch story content and scope from KB
...
- Story scope matches `kb_get_story({ story_id: "{STORY_ID}" })` content exactly
...
### Primary Output: ELAB.yaml (KB artifact)
